...
| 2   | DHT22 direct sensor read                                 | Python returns JSON with temperature and humidity                                                    | `npm test` returned a live reading `{"temperature": 31.4, "humidity": 79.7}` (one-off console check; ephemeral, not retained as a file)                                                  | Pass   | Live `npm test` console output (not retained); retained statistical run is the 30-read log in TC3 → [`./media/logs/iot1-sensor-30.log`](./media/logs/iot1-sensor-30.log)                                                                                                                                                                                                                                                           |
